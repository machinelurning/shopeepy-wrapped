...
class ShopeeInsights:
    def __init__(self, orders: Tuple[Dict]) -> None:
        ...
        self.completed_orders = df_orders.loc[df_orders["order_status"] == 'ORDER COMPLETED']
        self.products = df_products.merge(self.completed_orders[["order_id", "order_status"]], on="order_id")
        self.insights: Dict = {}
    ...
    def save_datasets(self) -> None:
        save_dataset(file_name=config.data_config.CLEAN_ORDERS_DF_FILENAME, dataset=self.orders)
        save_dataset(file_name=config.data_config.CLEAN_PRODUCTS_DF_FILENAME, dataset=self.products)
    ...
    def count_orders(self) -> None:
        try:
            ...
            self.update_insights_dict(["cnt_orders_cancelled", "cnt_orders_completed", "total_checkouts"],
    ...
    def get_avg_order_amt(self) -> None:
        try:
            ...
            self.update_insights_dict(["avg_order_amt"],
                                      [avg_order_amt])
        except:
            pass
    ...
    def sum_voucher_savings(self) -> None:
        try:
            shopee_voucher_savings = self.completed_orders[
                "shopee_voucher_applied"].sum()

            shop_voucher_savings = self.completed_orders[
                "shop_voucher_applied"].sum()

            total_voucher_savings = shop_voucher_savings + shopee_voucher_savings

            self.update_insights_dict(["shopee_voucher_savings", "shop_voucher_savings", "total_voucher_savings"],
    ...
    def update_insights_dict(
            self, key_list: List[Any], value_list: List[Any]
    ) -> None:
